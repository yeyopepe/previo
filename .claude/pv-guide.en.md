...
- [Setup](#setup)
  ...
  - [2. Initialize the framework: `/pv-init`](#2-initialize-the-framework-pv-init)
    ...
    - [Configuring the language: `interaction`/`changes`/`versions`/`docs.*.language`](#configuring-the-language-interactionchangesversionsdocslanguage)
...
#### Configuring the language: `interaction`/`changes`/`versions`/`docs.*.language`
...
`framework` can carry up to five independent `language` fields, all optional:
...
- **`interaction.language`** — the language skills use to talk to you in chat (questions, confirmations, summaries). It's also the fallback for every other field below when it isn't set.
- **`changes.language`** — language of an in-progress change/fix's documents: `description.md`, `plan.md`, `history.md`, and the sample text in `design_*.html`/`design_*.txt` mockups.
- **`versions.language`** — language of `changelog.md`, generated by `pv-version`/`pv-internal-changelog` from `changes/closed`. It's a separate family from `changes.language`: a changelog you publish externally can reasonably be in a different language than the one your team uses to document changes internally.
- **`docs.functional.language`** — language of `docs.functional.featuresDocPathDir`.
- **`docs.tech.language`** — language shared by `docs.tech.architectureDocDir` and `docs.tech.styleBibleDocDir`.

`pv-init` always asks about language on a first-time setup, proposing English as the default for `interaction` and offering to reuse the same value for the rest unless you want something different. If you initialized this project before language support existed, the next time you run `pv-init` it asks just this, without repeating the rest of the questionnaire.
