...
`pv-internal-workflow`, `pv-internal-tech-analysis`, `pv-internal-tech-security` y `pv-internal-changelog` solo se ejecutan cuando otra skill del framework las invoca como parte de su propio proceso; si el usuario las invoca directamente (o pide "ejecuta X" en texto plano sin venir de ese contexto), se detienen sin hacer nada y redirigen a la skill correspondiente.
...
- **Dos opciones mutan estado**, ambas con confirmación explícita (`y`/`N`) antes de ejecutar nada:
  ...
  - *Sincronizar modelos de las skills* (dentro del submenú *Configuración*): delega en `sync-skill-models.py` de `pv-init`, que propaga `skillModels` de `pv-context.json` al frontmatter (`model`/`effort`) de cada `SKILL.md` `pv-*`.
- **Submenús anidados** para opciones que agrupan más de una acción relacionada: *Configuración* (por ahora solo *Sincronizar modelos de las skills*) y *Comprobar versiones* — *Listar versiones y leer su changelog* (lista `{workFolder}/versions/{XXXX}/` y muestra el `changelog.md` de la elegida) y *Comprobar que `changes/closed/temp/` está vacío* (que quede algo ahí significa que una ejecución de `pv-version` falló a medias o sigue en marcha — ver paso 1 de `pv-internal-changelog` — y esta opción avisa de ello y lista lo que ha quedado atascado). Ambas opciones de este submenú son de solo lectura. Cada submenú tiene su propia opción "Volver" para regresar al menú raíz, que conserva "Salir".
- **Punto de extensión único**: si en el futuro conviene exponer algún otro script directamente en el menú (o submenú), debe ser puramente de lectura, o una mutación igual de simple y ya validada por su propio script (como mover una carpeta) y confirmada explícitamente antes de ejecutar. Mutaciones más complejas (borrar, crear versiones, redactar ficheros con contenido) quedan fuera del alcance de `pv.py` — necesitan contexto que solo la skill correspondiente puede aportar, así que siguen requiriendo Claude Code.
