...
5. **Indicar el siguiente paso.** Informa al usuario de que el cambio queda documentado (`description.md`) y, si procede, con su propuesta visual ya validada (`design_*.html`, `design_navigation_*.md`); para planificarlo e implementarlo debe invocar la skill `pv-how` sobre ese `xxxx`. Si el usuario quiere implementarlo ya mismo, puedes invocar `pv-how` directamente tú.
