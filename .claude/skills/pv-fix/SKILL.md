...
## 4.1 Definir los datos necesarios (fix no trivial)

Si el fix define o usa algo que necesita una lista de propiedades o datos asociados (propiedades de un objeto, contenido de una tabla de base de datos, campos de una configuración, etc.), escribe uno o varios ficheros `design_data_<descripción>.md` directamente en `{changesDir}/inProgress/{xxxx}/` — uno por cada entidad o conjunto de datos claramente diferenciado. Cada fichero contiene una o varias tablas Markdown que enumeran esos datos (columnas orientativas: nombre del dato, descripción funcional, obligatoriedad/valores posibles según aplique). Es **una definición funcional de qué datos hacen falta**, no de cómo se guardan o manipulan — eso lo decide `pv-how` después a partir de esta tabla. Si el fix no necesita ninguna lista de propiedades o datos estructurados, omite este paso por completo.

## 5. Validar la representación visual con el usuario (fix no trivial)

Si el paso 3 incluyó algún diagrama Mermaid, el paso 4 generó algún `design_*.html` o el paso 4.1 generó algún `design_data_*.md`, preséntaselos al usuario (ruta de cada `design_*.html`/`design_data_*.md` y el diagrama) y pídele que confirme si reflejan el comportamiento esperado o qué cambiaría, antes de encadenar la planificación. Si pide cambios, ajusta y vuelve a presentarlo hasta que lo confirme. Si el fix no generó ningún diagrama, `design_*.html` ni `design_data_*.md`, omite este paso.
...
No escribas tú mismo el documento de fix ni calcules el número `xxxx` — eso lo hace `pv-internal-workflow` para mantener un único sitio con esa lógica. Los ficheros `design_*.html` los genera la skill de maquetas configurada (`pv-internal-mockups-html` por defecto) — no los escribas tú mismo. El código de los diagramas Mermaid del paso 3 lo genera la skill de diagramas configurada (`pv-internal-tech-mermaid` por defecto) — tampoco lo redactes tú mismo. Los ficheros `design_data_*.md` (paso 4.1), en cambio, sí los escribes tú directamente — no son responsabilidad de ninguna skill interna. No escribas tú mismo el `plan.md` ni toques código directamente — eso lo hacen `pv-how` y `pv-do` para mantener un único sitio con esa lógica.
